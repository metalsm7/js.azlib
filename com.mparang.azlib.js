...
try { exports.node = azlib; } catch (e) { };
/*
try {
    var _azfc_style_obj = document.createElement('STYLE');
    _azfc_style_obj.innerHTML = azfc.values.css3.transaction + azfc.values.css3.shadow + azfc.values.css3.border + azfc.values.css3.button;
    if ((document.body == undefined || document.body == null) && document.head != null) {
        document.head.appendChild(_azfc_style_obj);
    } else if (document.body != null) {
        azfc.append(_azfc_style_obj);
    }
} catch (e) {}
*/
...
azlib.String = {
	init: function(p_value) {
		var azString = function(p_value) {
			...
			this.format = function(p_input_format, p_output_format) {
				if (azlib.isNull(value)) {
					throw new Error('source string is null.');
					return;
				}

				if (typeof(p_input_format) != 'string' || typeof(p_output_format) != 'string') {
					throw new Error('arguments is not string.');
					return;
				}

				if (value.length != p_input_format.length) {
					throw new Error('source string and input format string length is not equal.');
					return;
				}

				return this.set(azlib.string.toFormatString(value, p_input_format, p_output_format));
			};

			this.currency = function() {
				return azlib.string.toCurrencyString(value);
			}

			this.replaceAll = function(p_target, p_replace) {
				return azlib.string.replaceAll(value, p_target, p_replace);
			};

			this.reverse = function() {
				return azlib.string.toReverseString(value);
			};

			this.hasChars = function(p_value) {
				return azlib.string.isCompareStringOnly(value, p_value);
			};

			this.trimLeft = function() {
				return azlib.string.trimLeft(value);
			};

			this.trimRight = function() {
				return azlib.string.trimRight(value);
			};

			this.trim = function() {
				return azlib.string.trim(value);
			};

			this.byteLength = function() {
				return azlib.string.getByteLength(value);
			};
		}

		return new azString(p_value);
	}
};
*/


//alert('test:' + azlib('in_text'));
//alert('test:' + azlib('in_text').value);
//alert('test:' + azlib('in_text').get);
//alert('test:' + azlib.init());
//alert('test:' + azlib('in_text').is_array);
//alert('test:' + azlib('in_text').get().value);
...
azlib = function() {
	...
	if (this.arguments != null && this.arguments != undefined) {}
		if (this.arguments.length < 1) {
			throw new Error('azlib function needs arguments at least one.');
			return;
		}
	}
	...
	if (typeof(this.$object) == 'string') {
        ...
        if (document.getElementsByName(this.$object).length > 1) {
            // 동일한 id값을 가지는 객체의 갯수가 2개 이상인 경우
            this.$object = document.getElementsByName(this.$object);
            this.is_array = true;
        } 
        else {
            // 유일한 id인 경우
            ...
            if (this.$object == undefined || this.$object == null) {
                if (document.getElementsByName(this.$object).length == 1) 
                    this.$object = document.getElementsByName(this.$object)[0];
            }
        }
